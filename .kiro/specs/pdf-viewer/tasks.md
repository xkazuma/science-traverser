...
- 事後修正（ドロワーのスクロールバー非表示、kiro-debug 起点）: アウトライン/サムネイルのスクロール容器 `.pdf-sidebar-pane`（PdfSidebar）と `.pdf-thumbnail-list`（PdfThumbnail）に `scrollbar-width:none` / `-ms-overflow-style:none` / `::-webkit-scrollbar{display:none}` を追加し、ブラウザ標準スクロールバーを非表示化（`overflow-y:auto` は維持＝スクロール機能は残す）。本文 `.pdf-viewport` は対象外（スクロールバー維持）。CSS のみ・要件変更なし。
- 新機能（要件 9 ハイパーリンク/相互参照、kiro-debug 起点）: ページ3層に**注釈（リンク）層**を追加（canvas→text→**annotation**→overlay）。境界に `AnnotationLayer` を再エクスポート。`usePdfAnnotationLayer`＝`page.getAnnotations({intent:'display'})` + `new AnnotationLayer({div,page,viewport:viewport.clone({dontFlip:true}),linkService,annotationStorage})` + `render({annotations,renderForms:false})`、`--total-scale-factor` を viewport.scale に設定（テキスト層と同じ整合）。`usePdfLinkService`＝IPDFLinkService 最小実装、内部 dest は `usePdfOutline.resolveDest` 再利用で 1-origin ページ解決→`requestGoToPage`、外部 URL は `addLinkAttributes` で `target=_blank rel=noopener noreferrer`、named action 対応。`.annotation-layer` は `pointer-events:none`／リンク `<a>` のみ `auto`（リンク以外はテキスト選択維持）。linkService は store を遅延解決（Pinia 無しの PdfPage.spec を壊さない）。E2E テスト＝実リンク PDF（内部 dest→2ページ）をクリックし `pendingScrollTo===2` を検証。**教訓: pdfjs リンクは AnnotationLayer + IPDFLinkService 実装が必須、内部 dest 解決は resolveDest 再利用。**
